...
public class ReceiveHelper {
	...
	public void receiveMessages(Duration timeout, boolean returnOnTimeout, Integer maxMessages,
			Manager.ReceiveMessageHandler handler) throws IOException {
	...
	private List<HandleAction> retryFailedReceivedMessage(final Manager.ReceiveMessageHandler handler,
		...
		// If successful and for all other errors that are not recoverable, delete the
		// cached message
		if (!account.isDisconnected()) {
			cachedMessage.delete();
		}
